...
if (isset($result->message)) {
    ...
    $tgUsername = (isset($message->from->username)) ? "@" . $message->from->username : "";
    $tgUsernamePlain = (isset($message->from->username)) ? $message->from->username : "";
    $tgUsernameForChat = (isset($message->from->username)) ? "@" . str_replace("_", "###", $message->from->username) : "";
    ...
    if (in_array($userId, $admin_id) || isset($result->callback_query->data)) {
        ...
        if ($u) {
            ...
            if (
                ...
                // Include user reply configuration functions
    ...
    if (in_array($action, array(_l('button_start'), _l('menu_refresh'), _l('button_back_home'), _l('button_cancel')))) {
        ...
        if ($is_join <= 0) {
            ...
            prepareMessage(null, _l('already_registered'));
            exit();
        } else {
    ...
    else if (isset($message->contact->phone_number)) {
        $fChar = substr($message->contact->phone_number, 0, 1);
        if ($fChar != '+' && $fChar != '0') {
     ...
     * Share Selfie -> Request Input ID/Code/Email Employee **/
    else if (isset($message->photo) || (isset($message->document->mime_type) && in_array($message->document->mime_type, array('image/png', 'image/jpeg')))) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        ...
        if (count($user_data) > 0 && ($user_data['approval_status'] == 'approved' || $user_data['approval_status'] == '')) {
            ...
            if (
                ...
                $selfie_id = $message->photo[0]->file_id;
                ...
                $ezzeTeamsModel->updateUserPhoto($message->message_id);
                $ezzeTeamsModel->updateUserPhotoID($message->photo[0]->file_id);
            ...
            if (isTimeWorkingNow($user_data['user_id'])) {
                if (strtolower($action) == strtolower(_l('button_clock_out'))) {
                    if ($user_data['step'] != 'clock_in_done' && $user_data['step'] != 'on_break' && $user_data['step'] != 'on_visit') {
                        prepareMessage(null, _l('not_clock_in'));
                        exit;
     ...
     * Share Selfie -> Request Input ID/Code/Email Employee **/
    ...
    } else if (isset($message->location->live_period)) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        ...
        if (count($user_data) > 0 && $user_data['approval_status'] == 'approved') {
            ...
            if (
                ...
                $location_msg_id = $message->message_id;
                ...
                $params['reply_location_msg_id'] = $message->message_id;
    ...
    } else if (isset($message->location->latitude) && isset($message->location->longitude)) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        if (
            $user_data['step'] == 'clock_in_live_location' || $user_data['step'] == 'clock_out_live_location' ||
