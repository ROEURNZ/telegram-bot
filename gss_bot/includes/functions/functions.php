        ...
        if ($clock['is_clock_in'] == 'absent') {
            ...
            if ($clock['is_clock_in'] == 'clock_in') {
                ...
                $arr[$clock['user_id']] = [
                    ...
                    $break['total_break'],
                    $visit['total_visit'],
                    $clock['work_start_time'],
                    $clock['clock_in_time_status'],
                    $clock['clock_in_time'],
                    $alertClock,
                    $clockInDiffTime,
                    'CS',
                    $clock['clock_in_location_status'],
                    $gps,
                    $alertLocation,
                    $gpsDistance,
                    'CS'
...
{
    // Reference to a global language array $langBase which holds key-value pairs of translations
    global $langBase;

    // Initialize the return value as an empty string
    $val = '';

    // If the translation key is not found in $langBase, return the key itself (as a fallback)
    if (!isset($langBase[$key])) {
        return $key;
    }

    // Retrieve the translation string associated with the key
    $originalWords = $langBase[$key];

    // Count the number of parameters passed to the function
    $numParams = count($params);

    // Count how many placeholders (%s) are in the translation string
    $numVars = substr_count($originalWords, '%s');

    // Split the translation string into an array of words
    $x = explode(' ', $originalWords);

    // If the translation string contains more than 0 words
    if ($x > 0) {
        // Initialize a counter to keep track of which parameter to use
        $k = 0;

        // Iterate over each word in the translation string
        for ($i = 0; $i < count($x); $i++) {
            // Get the current word
            $var = $x[$i];

            // If the word is a placeholder (%s), replace it with the next parameter value
            if ($x[$i] == '%s') {
                // Replace the placeholder with the corresponding value from the $params array
                $var = $params[$k];
                // Increment the counter to move to the next parameter for future replacements
                $k++;
            }

            // Append the word (or replaced value) to the final result string with a space
            $val .= $var . " ";
        }
    }

    // Return the final result after trimming any extra spaces at the end
    return trim($val);
}
