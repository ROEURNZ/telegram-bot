...
class EzzeTeamsModel
{
    ...
    function saveUserBotRequest2($user_id, $params)
    {
        ...
        $sql = "INSERT INTO request_reply_log SET 
                ...
                created_at = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param('iss', $user_id, $userReq, $dateNow);
        $stmt->execute();
        return $stmt->insert_id;
    }


    function logReply($url, $params, $response, $logID = null)
    {
        ...
        $sql = "SELECT * FROM request_reply_log WHERE id = ? AND bot_reply IS NULL";
        $stmt = $mysqli->prepare($sql);
    ...
    // Here is where Register user admin function set up
    ...
    function updateDailyTask($task_id, $params)
    {
        ...
        $stmt->bind_param(
            ...
            $params['task_status'],
            $params['reply_time'],
            $params['reply_location'],
            $params['reply_location_status'],
            $params['reply_location_distance'],
            $params['reply_location_msg_id'],
            $task_id
        );
        return $stmt->execute();
    }
    ...
    /** I think from this line to the bottom are designed by different developer */
