...
if (isset($uData['lang']) && file_exists(__DIR__ . '/includes/language/' . $uData['lang'] . '.php')) {
    ...
    $userLang = "en";
    include __DIR__ . "/includes/language/en.php"; // English is default if there is no selected language
}
...
if (in_array($userId, $admin_id) || isset($result->callback_query->data)) {
    ...
    if ($u) {
        ...
        if (
            !isset($userCommandss['message']['entities'][0]['type']) ||
            ...
            if ($isCorrect === false) {
                prepareMessage(null, language('incorrect_reply'));
    ...
    if (in_array($action, array(language('button_start'), language('menu_refresh'), language('button_back_home'), language('button_cancel')))) {
        ...
        if ($is_join <= 0) {
            ...
            prepareMessage(array(array(language('button_lang_khmer'), language('button_lang_english'))), $selectLang);
            ...
            prepareMessage(array(array(language('button_back_home'))), language('restricted'));
        } else if ($user_data['step'] == 'waiting') {
            prepareMessage(null, language('already_registered'));
            exit();
        } else {
            prepareMessage(array(array(language('button_lang_khmer'), language('button_lang_english'))), language('select_language'));
        }
    }

    /***
     * Change Language After Clock In **/
    else if (strtolower($action) == strtolower(language('button_change_language')) || strtolower($action) == strtolower(language('command_change_lang'))) {
     ...
     * Select Khmer Language -> Back To English **/
    else if ($action == strtolower(language('button_lang_khmer'))) {
        $is_join = $ezzeTeamsModel->getUser();
        $user_data = $ezzeTeamsModel->getUser('', false);
        ...
        if ($user_data['approval_status'] == 'approved') {
            ...
            prepareMessage($st['keyboard'], '<strong>' . language('you_have_selected_kh') . '</strong>');
            ...
            prepareMessage(array(array(array('text' => language('button_share_contact'), 'request_contact' => true))), language('share_contact_msg'));
        }

        //prepareMessage(array(array(language('button_back_to_english'))), language('lang_not_available'));
    }

    /*** Step 3:
     * Select English Language -> Request Share Telegram Profile **/
    else if (in_array(strtolower($action), array(strtolower(language('button_lang_english')), strtolower(language('button_back_to_english'))))) {
        ...
        if ($user_data['approval_status'] == 'approved') {
            $st = getLastStatusByData($user_data['user_id']);
            prepareMessage($st['keyboard'], '<strong>' . language('you_have_selected_eng') . '</strong>');
     ...
     * Share Contact Information -> Request Share Selfie **/
        ...
        prepareMessage(array(array(language('button_cancel'))), language('please_send_selfie'));
    }
     ...
     * Share Selfie -> Request Input ID/Code/Email Employee **/
        ...
        if (count($user_data) > 0 && ($user_data['approval_status'] == 'approved' || $user_data['approval_status'] == '')) {
            ...
            if (
                $user_data['step'] == 'clock_in_req_selfie' || $user_data['step'] == 'clock_out_share_selfie' ||
                ...
                if ($user_data['step'] == 'clock_out_share_selfie') {
                    ...
                    if ($clock_in_data['clock_in_location_status'] != 'OK' && $clock_in_data['clock_in_time_status'] != 'OK') {
                        if ($clock_in_data['clock_in_time_status'] == 'LATE CLOCK OUT') {
                            $m = language('clock_out_wrong_and_late', [$first_name . ' ' . $last_name]);
                        } else {
                            $m = language('clock_out_wrong_and_early', [$first_name . ' ' . $last_name]);
                        }
                        $employeeMsg = language('clock_out_received');
                        $sentNotifications = true;
                    } else if ($clock_in_data['clock_in_time_status'] != 'OK') {
                        if ($clock_in_data['clock_in_time_status'] == 'EARLY CLOCK OUT') {
                            $m = language('clock_out_early', [$first_name . ' ' . $last_name]);
                        } else {
                            $m = language('clock_out_late', [$first_name . ' ' . $last_name]);
                        }
                        $employeeMsg = language('clock_out_received');
                        $sentNotifications = true;
                    } else if ($clock_in_data['clock_in_location_status'] != 'OK') {
                        $m = language('clock_out_wrong_location', [$first_name . ' ' . $last_name]);
                        $employeeMsg = language('clock_out_received');
                        $sentNotifications = true;
                    } else {
                        $m = language('clock_out_ok', [$first_name . ' ' . $last_name]);
                        $employeeMsg = language('success_clock_out');
                    }
                    prepareMessage(array(array(language('button_clock_in'))), '<strong>' . $employeeMsg . '</strong>');
                    ...
                    prepareMessage($r['keyboard'], language('break_started'));
                    ...
                    $r = getMsgByStep($user_data);
                    prepareMessage($r['keyboard'], language('break_end'));
                    ...
                    prepareMessage(null, language('type_note'));
                } else {
                    ...
                    if ($clock_in_data['clock_in_location_status'] != 'OK' && $clock_in_data['clock_in_time_status'] != 'OK') {
                        if ($clock_in_data['clock_in_time_status'] == 'EARLY CLOCK IN') {
                            $m = language('clock_in_wrong_and_early', [$first_name . ' ' . $last_name]);
                        } else {
                            $m = language('clock_in_wrong_and_late', [$first_name . ' ' . $last_name]);
                        }
                        $employeeMsg = language('clock_in_received');
                        $sentNotifications = true;
                    } else if ($clock_in_data['clock_in_time_status'] != 'OK') {
                        if ($clock_in_data['clock_in_time_status'] == 'EARLY CLOCK IN') {
                            $m = language('clock_in_early', [$first_name . ' ' . $last_name]);
                        } else {
                            $m = language('clock_in_late', [$first_name . ' ' . $last_name]);
                        }
                        $employeeMsg = language('clock_in_received');
                        $sentNotifications = true;
                    } else if ($clock_in_data['clock_in_location_status'] != 'OK') {
                        $m = language('clock_in_wrong_location', [$first_name . ' ' . $last_name]);
                        $employeeMsg = language('clock_in_received');
                        $sentNotifications = true;
                    } else {
                        $m = language('clock_in_ok', [$first_name . ' ' . $last_name]);
                        $employeeMsg = language('clock_in_success');
                    }
                    $user_data['step'] = 'clock_in_done';
                    $r = getMsgByStep($user_data);
                ...
                prepareMessage(array(array(language('skip_and_send')), array(language('button_cancel'))), language('registration_enter_id'));
                exit;
            } else {
                //$r = getLastStatusByData($user_data['user_id']);
                prepareMessage(null, language('command_not_exists'));
                exit;
            }
        } else {
            prepareMessage(array(array(language('button_back_home'))), language('restricted'));
    ...
    } else if (strtolower($action) == strtolower(language('my_schedule')) || strtolower($action) == strtolower(language('button_my_schedule')) || strtolower($action) == strtolower(language('menu_schedule'))) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        if ($user_data['approval_status'] == 'approved') {
            $msg = language('admin_info_work_location') . ' ' . $user_data['branch_name'];
    ...
    } else if (strtolower($action) == strtolower(language('menu_edit_profile'))) {
        prepareMessage(null, language('menu_coming_soon_desc'));
        exit;
    } else if (strtolower($action) == strtolower(language('button_show_work_location')) || strtolower($action) == strtolower(language('menu_show_work_location'))) {
        $branch = $ezzeTeamsModel->getEmployeeBranchByUserId($userId);
        prepareLocationMessage([$userId], $branch['branch_lat'], $branch['branch_lon']);
        exit;
    } else if (strtolower($action) == strtolower(language('clock_out_now')) || strtolower($action) == strtolower(language('button_yes')) || strtolower($action) == strtolower(language('button_no'))) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        if (strtolower($action) == strtolower(language('clock_out_now')) || strtolower($action) == strtolower(language('button_yes'))) {
            ...
            if ($m == NULL || $m['is_clock_in'] == 'clock_out') {
                ...
                prepareMessage($r['keyboard'], language('not_clock_in'));
            } else {
                $ezzeTeamsModel->updateUserWhere($userId, "step = 'clock_out_live_location'");
                prepareMessage(null, language('share_live_location'));
            }
            exit;
        } else {
            $v = (int) 1;
            $ezzeTeamsModel->changeCompleteUserStep($user_data['user_id'], $v);
            $r = getLastStatusByData($user_data['user_id']);
            prepareMessage($r['keyboard'], language('thank_you'));
            exit;
        }
    } else if (strtolower($action) == strtolower(language('button_clock_in')) || strtolower($action) == strtolower(language('button_clock_out'))) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        if (!$user_data['is_step_complete']) {
            prepareMessage(null, language('not_completed_step'));
            exit;
        }

        if (count($user_data) > 0 && $user_data['approval_status'] == 'approved') {
            if (isTimeWorkingNow($user_data['user_id'])) {
                if (strtolower($action) == strtolower(language('button_clock_out'))) {
                    if ($user_data['step'] != 'clock_in_done' && $user_data['step'] != 'on_break' && $user_data['step'] != 'on_visit') {
                        prepareMessage(null, language('not_clock_in'));
                        exit;
                        ...
                        prepareMessage(array(array(language('button_yes')), array(language('button_no'))), language('clock_out_confirmation'), null, null, array($userId), $keyboard_config);
                        exit;
                    } else {
                        $ezzeTeamsModel->updateUserWhere($userId, "step = 'clock_out_done'");
                        $user_data['step'] = 'clock_out_done';
                        $r = getMsgByStep($user_data);
                        prepareMessage($r['keyboard'], language('already_clock_out'));
                        exit;
                    }
                } else if (strtolower($action) == strtolower(language('button_clock_in'))) {
                    if ($ezzeTeamsModel->isClockIn() < 1) {
                        ...
                        $r = getMsgByStep($user_data);
                        prepareMessage(generateClockInBtn(), language('already_clock_in'));
                        exit;
                    }
                }
                prepareMessage(null, language('share_live_location'));
            } else {
                if (strtolower($action) == strtolower(language('button_clock_in'))) {
                    prepareMessage(null, language('failed_clock_in'));
                } else {
                    prepareMessage(null, language('failed_clock_out'));
                }
            }
        } else {
            prepareMessage(array(array(language('button_back_home'))), language('restricted'));
        }
    } else if (strtolower($action) == strtolower(language('button_start_break')) || strtolower($action) == strtolower(language('button_end_break'))) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        $r = getMsgByStep($user_data);
        if ($botSettings['module_break'] == 1) {
            ...
            if (count($user_data) > 0 && $user_data['approval_status'] == 'approved') {
                if ($user_data['can_break'] == 1) {
                    ...
                    if (strtolower($action) == strtolower(language('button_start_break'))) {
                        if ($user_data['break_step'] == 1) {
                            if ($user_data['step'] == 'clock_in_done' || $user_data['step'] == 'start_break_req_location' || $user_data['step'] == 'start_break_req_selfie') {
                                ...
                                $r = getMsgByStep($user_data);
                                prepareMessage($r['keyboard'], language('already_break'));
                            } else {
                                prepareMessage(array(array(language('button_clock_in'))), language('not_clock_in'));
                            }
                        } else {
                            $ezzeTeamsModel->updateUserWhere($userId, "step = 'on_break'");
                            $params = [];
                            ...
                            $user_data['step'] = 'on_break';
                            $r = getMsgByStep($user_data);
                            prepareMessage($r['keyboard'], language('start_break'));
                            exit;
                        }
                    } elseif (strtolower($action) == strtolower(language('button_end_break'))) {
                        if ($user_data['break_step'] == 1) {
                            if ($user_data['step'] == 'on_break' || $user_data['step'] == 'end_break_req_location' || $user_data['step'] == 'end_break_req_selfie') {
                                ...
                                prepareMessage(null, language('not_in_break'));
                            }
                        } else {
                            $ezzeTeamsModel->updateUserWhere($userId, "step = 'clock_in_done'");
                            $params = [];
                            ...
                            $user_data['step'] = 'clock_in_done';
                            $r = getMsgByStep($user_data);
                            prepareMessage($r['keyboard'], language('success_break'));
                            exit;
                        }
                    } else {
                        prepareMessage(null, language('command_not_exists'));
                    ...
                    prepareMessage(null, language('employee_disabled_module'));
                }
            } else {
                prepareMessage(null, language('restricted'));
            }
        } else {
            prepareMessage(null, language('restricted_module'));
        }
    } else if (strtolower($action) == strtolower(language('button_start_visit')) || strtolower($action) == strtolower(language('button_end_visit'))) {
        $user_data = $ezzeTeamsModel->getUser('', false);
        $r = getMsgByStep($user_data);
        if ($botSettings['module_visit'] == 1) {
            ...
            if (count($user_data) > 0 && $user_data['approval_status'] == 'approved') {
                if ($user_data['can_visit'] == 1) {
                    ...
                    if (strtolower($action) == strtolower(language('button_start_visit'))) {
                        if ($user_data['step'] == 'clock_in_done' || $user_data['step'] == 'start_visit_req_location' || $user_data['step'] == 'start_visit_req_selfie') {
                            ...
                            $r = getMsgByStep($user_data);
                            prepareMessage($r['keyboard'], language('already_visit'));
                        } else {
                            prepareMessage(array(array(language('button_clock_in'))), language('not_clock_in'));
                        }
                    } elseif (strtolower($action) == strtolower(language('button_end_visit'))) {
                        if ($user_data['step'] == 'on_visit') {
                            ...
                            $r = getMsgByStep($user_data);
                            prepareMessage($r['keyboard'], language('not_in_visit'));
                        }
                    } else {
                        prepareMessage(null, language('command_not_exists'));
                    }
    ...
    /** reminder feature */
    else if (strtolower($action) == strtolower(language('button_remind_later'))) {
        ...
        if (is_array($reminderData) && count($reminderData) > 1) {
            ...
            $msg = language('remind_me_again', [$botSettings['clockout_reminder_interval']]);
     ...
     * Share Selfie -> Request Input ID/Code/Email Employee **/
        ...
        if (count($user_data) > 0 && $user_data['approval_status'] != 'rejected') {
            if ($user_data['step'] == 'register_req_photo') {
                prepareMessage(null, language('need_share_selfie'));
                ...
                prepareMessage(array(array(language('button_back_home'))), language('success_registration'));
                ...
                prepareMessage(array(array(language('button_show_user_table'), language('button_show_employee'))), getReceivedNewApplicationNotificationTxt(), null, null, $admin_id);
                //    foreach ($admin_id as $id) {
                ...
                $r = getMsgByStep($user_data);
                prepareMessage($r['keyboard'], language('start_visit'));
                ...
                $r = getMsgByStep($user_data);
                prepareMessage($r['keyboard'], language('end_visit'));
        ...
        if (count($user_data) > 0 && $user_data['approval_status'] == 'approved') {
            ...
            if (
                $user_data['step'] == 'clock_in_live_location' || $user_data['step'] == 'clock_out_live_location' ||
                ...
                prepareMessage(null, language('please_send_selfie'));
                ...
                prepareMessage(null, language('deadman_task_received'));
                if ($tasks > 0) {
                    ...
                    if ($location_status == 'WRONG LOCATION') {
                        ...
                        $msg = "<strong>" . $user_data['firstname'] . " " . $user_data['lastname'] . "</strong> " . language('failed_ping_tasks') .
                            "_" . language('admin_info_ping_time') . date("H:i", strtotime($tasks['task_start'])) . " - " . date("H:i", strtotime($tasks['task_end'])) .
                            "__" . language('admin_info_job_description') . $user_data['jobdesc'] .
                            "_" . language('admin_info_work_location') . $user_data['branch_name'] .
                            "__" . language('admin_info_first_name') . $user_data['firstname'] .
                            "_" . language('admin_info_last_name') . $lastname .
                            "_" . language('admin_info_phone_number') . $phone .
                            "_" . language('admin_info_employee_id') . $eId .
                            "__" . language('admin_info_distance') . $distance .
                            "_" . language('admin_info_map') . "/showMapPing" . $tasks['id'] .
                            "_" . language('admin_info_employee') . "/viewEmployee" . $user_data['user_id'];
                        /*
                ...
                prepareMessage(null, language('select_action'));
            }
        } else {
            prepareMessage(array(array(language('button_back_home'))), language('restricted'));
        }
        ...
        if (
            ...
            prepareMessage(null, language('use_live_not_share_location'));
        } else {
            prepareMessage(null, language('command_not_exists'));
        }
    }
