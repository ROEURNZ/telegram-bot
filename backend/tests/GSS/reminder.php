        ...
        if ($list['reminder_num'] > 1) {
            ...
            if ($list['reminder_num'] < $botSettings['max_reminder']) {
                ...
                /*if (!$canReply) {
                    ...
                    prepareMessage($btn, language('remind_force_clock_out_not_reply'), null, null, [$list['user_id']]);
                } else {*/
                    $btn = null;
                ...
                $ezzeTeamsModel->setReminderSent($list['id']);
                $btn = generateButtonClockOutNow();
                prepareMessage($btn, language('remind_clockout_last'), null, null, [$list['user_id']]);
            }
        }
    }
    ...
    foreach ($lists as $list) {
        if ($list['type'] == 'clock_out') {
            ...
            if ($list['reminder_num'] >= $botSettings['max_reminder']) {
                ...
                forceClockOut($data);
                prepareMessage($btn, language('remind_force_clock_out'), null, null, [$list['user_id']]);
            } else {
                /*$canReply = isLastFiveReminderNotReplyByUser($list['user_id'], $dateNow);
                if (!$canReply) {
                    //send note force clock out cause not reply 5 times
                    $btn = generateButtonClockOutNow();
                    forceClockOut($data);
                    prepareMessage(generateClockInBtn(), language('remind_force_clock_out_not_reply'), null, null, [$list['user_id']]);
                }*/
                forceClockOut($data);
                prepareMessage(generateClockInBtn(), language('remind_force_clock_out'), null, null, [$list['user_id']]);
            }
        }
    }
}