...
if ( in_array($action, array(language('button_start'), language('button_back_home'), language('button_cancel'))) ) {
    $ezzeTeamsModel->setAdminStep($userId, '', '');
    prepareMessage(array(array(language('button_show_user_table'), language('button_show_employee'))), language('admin_account'));
}
    ...
    elseif (strpos($action, 'Ping') !== false ) {
        ...
        prepareMessage(null, language('admin_command_not_exists'));
    }
}
    ...
    if ($adminStep['step'] == 'user_approve_jobdesc') {
        ...
        prepareMessage(array(array(language('button_skip'))), language('admin_employee_note', [$params->employee_name]));
    }
    else if ($adminStep['step'] == 'user_approve_notes') {
        $notes = (strtolower($result->message->text) == strtolower(language('button_skip')))? '' : ucwords($result->message->text);
        $params = json_decode($adminStep['temp']);
        ...
        prepareMessage(array(array(language('button_show_user_table'), language('button_show_employee'))), language('admin_information_saved'));

        if ($botSettings['module_alert'] == 1) {
            $inline = generateImportantUserBtn($params->employee_id, 'user');
            prepareMessage(null, '• ' . language('admin_registration_trigger_alarm') . ' <strong>' . $params->employee_name . "</strong>?", null, null, array($userId), null, true, $inline);
        } else if ($botSettings['module_break'] == 1) {
            $inline = generateBreakModuleUserBtn($params->employee_id, 'user');
            prepareMessage(null, '• ' . language('admin_registration_trigger_break') . ' <strong>' . $params->employee_name . "</strong>?", null, null, array($userId), null, true, $inline);
        } else if ($botSettings['module_visit'] == 1) {
            $inline = generateVisitModuleUserBtn($params->employee_id, 'user');
            prepareMessage(null, '• ' . language('admin_registration_trigger_visit') . ' <strong>' . $params->employee_name . "</strong>?", null, null, array($userId), null, true, $inline);
        } else if ($botSettings['dead_man_feature'] == 1) {
            $inline = generatePingModuleUserBtn($params->employee_id, 'user');
            prepareMessage(null, '• ' . language('admin_registration_ping_module') . ' <strong>' . $params->employee_name . "</strong>?", null, null, array($userId), null, true, $inline);
        } else {
...
/** Admin functions config bot **/
    ...
    $c = explode(' ', strtolower($action));
    $defCommand = "_".language('admin_accepted_cmd').": " .
        "<code>" .
        "_/config show" .
        "_/config time-tolerance minutes(0-45)" .
        "_/config distance-tolerance meters(0-500)" .
    ...
    if (strtolower($c[1]) == 'show') {
        $alertModule = ($botSettings['module_alert'])?language('enabled'):language('disabled');
        $breakModule = ($botSettings['module_break'])?language('enabled'):language('disabled');
        $visitModule = ($botSettings['module_visit'])?language('enabled'):language('disabled');
        $pingModule = ($botSettings['dead_man_feature'])?language('enabled'):language('disabled');
        $msg = language('admin_bot_info_modules').
                "_". language('admin_bot_module_alert'). $alertModule .
                "_". language('admin_bot_module_break'). $breakModule .
                "_". language('admin_bot_module_visit'). $visitModule .
                "_". language('admin_bot_module_ping'). $pingModule .
                "__". language('admin_bot_settings') .
                "_". language('admin_bot_time_tolerance') . $botSettings['time_tolerance']." min" .
                "_". language('admin_bot_distance_tolerance') . ($botSettings['location_tolerance'] * 1000)." m";

        if ($botSettings['dead_man_feature']) {
            $msg .= "_". language('admin_bot_ping_tolerance') . $botSettings['dead_man_task_time']." min";
        }
        ...
        */
        ...
        if (isset($c[2])) {
            ...
            if ($time >= 1 && $time <= 45) {
                ...
                $msg = language('admin_clocktime_success', [$time]);
            } else {
                $msg = language('admin_clocktime_range_error') . $defCommand;
            }
        } else {
            $time = 10;
            $ezzeTeamsModel->changeBotSetting('time_tolerance', $time);
            $msg = language('admin_clocktime_success', [$time]);
            //$msg = language('admin_clocktime_empty')." _" . $defCommand;
        }
    }
        ...
        if (isset($c[2])) {
            ...
            if ($distance >= 1 && $distance <= 500) {
                ...
                $msg = language('admin_clockdistance_success', [$distance]);
            } else {
                $msg = language('admin_clockdistance_range_error') . $defCommand;
            }
        } else {
            $distance = 25;
            $v = $distance / 1000;
            $ezzeTeamsModel->changeBotSetting('location_tolerance', $v);
            $msg = language('admin_clockdistance_success', [$distance]);
            //$msg = language('admin_clockdistance_empty')." _" . $defCommand;
        }
    }
            ...
            if (strtolower($c[3]) == 'true') {
                ...
                if (count($x) == 2) {
                    if ( $x[0] >= 0 && $x[0] < 24 && $x[1] >= 0 && $x[1] < 60 ) {
                        ...
                        if ($m) {
                            $msg = language('admin_schedule_absent_success', [$c[4]]);
                        } else {
                            $msg = language('admin_schedule_absent_failed');
                        }
                    } else {
                        $msg = language('admin_time_range_invalid');
                    }
                } else {
                    $msg = language('admin_time_invalid');
                }
            } elseif (strtolower($c[3]) == 'false') {
                $val = (int) 0;
                $m = $ezzeTeamsModel->changeCronActive(strtolower($c[2]), $val);
                $msg = language('admin_schedule_absent_off');
            } else {
                $msg = language('admin_command_not_exists')." _" . $defCommand;
            }
        }
        elseif (strtolower($c[2]) == 'daily-report') {
            if (strtolower($c[3]) == 'enable') {
                ...
                $msg = language('admin_schedule_daily_report_set_time');
            } elseif (strtolower($c[3]) == 'disable') {
                $val = (int) 0;
                $m = $ezzeTeamsModel->changeCronActive(strtolower($c[2]), $val);
                $msg = language('admin_schedule_daily_report_off');
            } else {
                $msg = language('admin_command_not_exists')." _" . $defCommand;
            }
        }
        ...
        if (strtolower($c[2]) == 'email') {
            if (strtolower($c[3]) != '') {
                if (!filter_var($c[3], FILTER_VALIDATE_EMAIL)) {
                    $msg = language('admin_invalid_email');
                } else {
                    $email = strtolower($c[3]);
                    $ezzeTeamsModel->setBotSetting('company_email', $email);
                    $msg = language('admin_company_email_success');
                }
            } else {
                $msg = language('admin_please_type_email');
            }
        }
        else if (strtolower($c[2]) == 'phone') {
            if (strtolower($c[3]) != '') {
                if(!filter_var($c[3], FILTER_SANITIZE_NUMBER_INT)) {
                    $msg = language('admin_invalid_phone');
                } else {
                    $phone = $c[3];
                    $ezzeTeamsModel->setBotSetting('company_phone', $phone);
                    $msg = language('admin_company_phone_success');
                }
            } else {
                $msg = language('admin_please_type_phone');
            }
        }
        else {
            $msg = language('admin_command_not_exists')." _".$defCommand;
        ...
        if (strtolower($c[2]) == 'alert') {
            if (strtolower($c[3]) == 'enable' || strtolower($c[3]) == 'disable') {
                if (strtolower($c[3]) == 'enable') {
                    $val = 1;
                    $msg = language('admin_module_on', ['Module Alert']);
                } else {
                    $val = 0;
                    $msg = language('admin_module_off', ['Module Alert']);
                };
                $ezzeTeamsModel->changeBotSetting('module_alert', $val);
            } else {
                $msg = language('admin_module_miss_cmd');
            }
        }
            ...
            if (strtolower($c[3]) == 'enable' || strtolower($c[3]) == 'disable') {
                if (strtolower($c[3]) == 'enable') {
                    $val = 1;
                    $msg = language('admin_module_on', ['Module Break']);
                } else {
                    $val = 0;
                    $msg = language('admin_module_off', ['Module Break']);
                };
            ...
            if (strtolower($c[3]) == 'enable' || strtolower($c[3]) == 'disable') {
                if (strtolower($c[3]) == 'enable') {
                    $val = 1;
                    $msg = language('admin_module_on', ['Module Visit']);
                } else {
                    $val = 0;
                    $msg = language('admin_module_off', ['Module Visit']);
                };
            ...
            if (strtolower($c[3]) == 'enable') {
                ...
                if (isset($c[4])) {
                    ...
                    if ($time >= 1 && $time <= 60 ){
                        ...
                        $msg = language('admin_deadman_success', [$c[4]]);
                    } else {
                        $msg = language('admin_deadman_timerange_error') . $defCommand;
                    }
                } else {
                    $time = 30;
                        $ezzeTeamsModel->setDeadManFeature($val, $time);
                        $msg = language('admin_deadman_success', [$time]);
                    //$msg = language('admin_deadman_timerange_empty') . $defCommand;
                }
            } elseif (strtolower($c[3]) == 'disable') {
                $val = 0;
                $ezzeTeamsModel->setDeadManFeature($val);
                $msg = language('admin_deadman_off_success');
            } else {
                $msg = language('admin_command_not_exists')." _" . $defCommand;
            }
        }
        ...
        if (strtolower($c[2]) == 'help') {
            $msg = language('admin_accepted_cmd').": " .
                "<code>" .
            ...
            if (isset($c[3])) {
                ...
                if ($time >= 30 && $time <= 60) {
                    ...
                    $msg = language('admin_reminder_interval_success', [$time]);
                } else {
                    $msg = language('admin_reminder_interval_range_error');
                }
            } else {
                $time = 30;
                $ezzeTeamsModel->changeBotSetting('clockout_reminder_interval', $time);
            ...
            if (isset($c[3])) {
                ...
                if ($time >= 1 && $time <= 29) {
                    ...
                    $msg = language('admin_reminder_timeout_success', [$time]);
                } else {
                    $msg = language('admin_reminder_timeout_range_error');
                }
            } else {
                $time = 10;
                $ezzeTeamsModel->changeBotSetting('clockout_reminder_timeout', $time);
    ...
    if ($adminStep['step'] == 'set-welcome-message') {
        if (isset($result->message->photo)) {
            ...
            $ezzeTeamsModel->setAdminStep($userId, '');
            prepareMessage(null, language('admin_welcome_message_saved'));
        }
        ...
        prepareMessage(array(array(language('button_show_user_table'), language('button_show_employee'))), $msg);
    ...
    if ($c[1] == 'add') {
        ...
        if ( $x[1] != '' ) {
            ...
            if ( count($branchExists) > 0 ) {
                $msg = language('admin_branch_already_exists');
            } else {
                ...
                $msg = language('admin_branch_add_location', [ucfirst($branchName)]);
            }
        } else {
            $msg = language('admin_branch_missing_name');
        }
    } else if ($c[1] == 'edit') {
        $cmd = "/branch edit";
        $x = explode($cmd, $action);
        if ($x != '') {
            ...
            if ( count($branchExists) > 0 ) {
                ...
                $msg = language('admin_branch_set_new_name', [ucfirst($branchExists['branch_name'])]);
            } else {
                $msg = language('admin_branch_not_exists');
            }
        } else {
            $msg = language('admin_branch_missing_name');
        }
        ...
        if ( $x[1] != '' ) {
            ...
            if ( count($branchExists) > 0 ) {
                ...
                $msg = language('admin_branch_remove_confirmation', [ucwords($branchExists['branch_name'])]);
                prepareMessage(array(array(language('button_yes')), array(language('button_no'))), $msg);
                return;
            } else {
                $msg = language('admin_branch_not_exists');
            }
        ...
        foreach ($branchLists as $b) {
            $msg .= "_".$i.". <strong>".$b['branch_name']."</strong>" .
                    "_". language('admin_info_latitude') . $b['branch_lat'] .
                    "_". language('admin_info_longitude') . $b['branch_lon'] .
                    "_". language('admin_info_map'). "/showMapBranch".$b['branch_id']."_";
            $i++;
        }
        prepareMessage(null, $msg);
        return;
    ...
    if ( in_array($adminUser['step'], $arr_step) ) {
        if ($adminUser['step'] == 'admin_branch_add_location') {
            ...
            $ezzeTeamsModel->setAdminStep($userId, '');
            $msg = language('admin_branch_add_success');
        }
        ...
        $msg = language('admin_info_is_correct')." __".ucwords($pos['new_branch_name']);
        prepareMessage(array(array(language('button_yes')), array(language('button_cancel'))), $msg);
        $params = [
            'chat_id' => $userId,
        ...
        $msg = language('admin_command_not_exists');
    }
    prepareMessage(null, $msg);
    return;
}
...
/** Generate CLock report */
    ...
    $defCommand = "_".language('admin_generate_report_cmd').": " .
    ...
    if (strtolower($c[1]) == 'standard') {
        if (strtolower($c[2]) == 'attendance') {
            ...
            if (isset($cm[1]) && $cm[1] != '') {
                ...
                if ($dateOk == false) {
                    $msg = language('admin_report_invalid_date') . " _" . $defCommand;
                } elseif ($branchName == '') {
                    $msg = language('admin_report_missing_branch') . " _" . $defCommand;
                } else {
                    $branch = $ezzeTeamsModel->getBranchByName($branchName);
                    if ($branchName != 'all' && count($branch) < 1) {
                        $msg = language('admin_report_missing_branch');
                        $msg .= "__" . $defCommand;
                    } else {
                        //validate date
                        ...
                        if (count($dtS) < 3 && count($dtE) < 3 && !checkdate($dtS[1], $dtS[2], $dtS[0]) && !checkdate($dtE[1], $dtE[2], $dtE[0])) {
                            $msg = language('admin_report_invalid_date');
                        } elseif ($dtSStr > $dtEStr) {
                            $msg = language('admin_report_date_validation');
                        } elseif ($dtSStr > $dtTStr || $dtEStr > $dtTStr) {
                            $msg = language('admin_report_date_max_today');
                        } else {
                            if ($branchName == 'all') {
                                ...
                                foreach ($branches as $bbranch) {
                                    ...
                                    $msg = language('admin_report_attendance') . " " . ucwords($bbranch['branch_name']) . " " . $textMsg;
                                ...
                                $msg = language('admin_report_attendance') . " " . ucwords($branchName) . " " . $textMsg;
                                $inline_keyboard = [
                                    [
                                        [
                ...
                $msg = language('admin_command_not_exists') . " " . $defCommand;
            }
        }
        elseif (strtolower($c[2]) == 'break') {
            ...
            if (isset($cm[1]) && $cm[1] != '') {
                ...
                if ($dateOk == false) {
                    $msg = language('admin_report_invalid_date') . " " . $defCommand;
                } elseif ($branchName == '') {
                    $msg = language('admin_report_missing_branch') . " " . $defCommand;
                } else {
                    $branch = $ezzeTeamsModel->getBranchByName($branchName);
                    if ($branchName != 'all' && count($branch) < 1) {
                        $msg = language('admin_report_missing_branch');
                        ...
                        if (count($dtS) < 3 && count($dtE) < 3 && !checkdate($dtS[1], $dtS[2], $dtS[0]) && !checkdate($dtE[1], $dtE[2], $dtE[0])) {
                            ...
                            if ($branchName == 'all') {
                                ...
                                foreach ($branches as $bbranch) {
                                    ...
                                    $msg = language('admin_report_break') . " " . ucwords($bbranch['branch_name']) . " " . $textMsg;
                                    prepareMessage(null, $msg, null, 'sendDocument', null, null, false, null, null, true, $cs);
                                    unlink($cs);
                                }
                                exit;
                                ...
                                $msg = language('admin_report_break') . " " . ucwords($branchName) . " " . $textMsg;
                                $inline_keyboard = [
                                    [
                                        [
                                            "text" => "Download CSV File",
                    ...
                    if (count($dtS) < 3 && count($dtE) < 3 && !checkdate($dtS[1], $dtS[2], $dtS[0]) && !checkdate($dtE[1], $dtE[2], $dtE[0])) {
                        ...
                        if ($branchName == 'all') {
                            ...
                            foreach ($branches as $bbranch) {
                                ...
                                $msg = language('admin_report_event') . " " . ucwords($bbranch['branch_name']) . " " . $textMsg;
                                prepareMessage(null, $msg, null, 'sendDocument', null, null, false, null, null, true, $cs);
                                unlink($cs);
                            }
                            exit;
                            ...
                            $msg = language('admin_report_event') . " " . ucwords($branchName) . " " . $textMsg;
                            $inline_keyboard = [
                                [
                                    [
                                        "text" => "Download CSV File",
...
/** View detail Approved Employee */
    ...
    if (!isset($user_data) || $user_data['approval_status'] == 'rejected') {
        prepareMessage(null, language('admin_user_not_found'));
        return;
    }
...
/** User Working Configurations */
    ...
    if ($data[0] == 'approve') {
        ...
        prepareMessage(null,  '• '.language('admin_registration_select_branch').' <strong>' . $name . "</strong>", null, null, array($result->callback_query->from->id), null, true, $inline_key_branch);
    }
        ...
        prepareMessage(null, language('admin_enter_jobdesc', [$name]),null, null, array($result->callback_query->from->id));
        ...
        if ($botSettings['module_break'] == 1) {
            ...
            prepareMessage(null, '• ' . language('admin_registration_trigger_break') . ' <strong>' . $name . "</strong>?", null, null, array($result->callback_query->from->id), null, true, $inline);
        } else if ($botSettings['module_visit'] == 1) {
            $inline = generateVisitModuleUserBtn($data[1], $data[2]);
            prepareMessage(null, '• ' . language('admin_registration_trigger_visit') . ' <strong>' . $name . "</strong>?", null, null, array($result->callback_query->from->id), null, true, $inline);
        } else if ($botSettings['dead_man_feature'] == 1) {
            $inline = generatePingModuleUserBtn($data[1], $data[2]);
            prepareMessage(null, '• ' . language('admin_registration_ping_module') . ' <strong>' . $name . "</strong>?", null, null, array($result->callback_query->from->id), null, true, $inline);
        } else {
        ...
        if ($data[3] == 1) {
            ...
            prepareMessage(null, '• ' . language('admin_registration_break_step') . ' <strong>' . $name . "</strong>?", null, null, array($result->callback_query->from->id), null, true, $inline);
        }
        else if ($botSettings['module_visit'] == 1) {
            $inline = generateVisitModuleUserBtn($data[1], $data[2]);
            prepareMessage(null, '• ' . language('admin_registration_trigger_visit') . ' <strong>' . $name . "</strong>?", null, null, array($result->callback_query->from->id), null, true, $inline);
        ...
        if ($data[3] == 1) {
            ...
            prepareMessage(null, '• ' . language('admin_registration_alert_visit') . ' <strong>' . $name . "</strong>?", null, null, array($result->callback_query->from->id), null, true, $inline);
        } else if ($botSettings['dead_man_feature'] == 1) {
            $inline = generatePingModuleUserBtn($data[1], $data[2]);
            prepareMessage(null, '• ' . language('admin_registration_ping_module') . ' <strong>' . $name . "</strong>?", null, null, array($result->callback_query->from->id), null, true, $inline);
        } else {
     ...
     */
        ...
        if ($nextDay > 7) {
            ...
            if ($employee_data['approval_status'] == 'approved') {
                ...
                $m = language('admin_change_working_hour_success')." <strong>".$employee_data['firstname']." ".$employee_data['lastname']."</strong>";
                $m .= $newSchedule;
                ...
                prepareMessage(null, language('admin_approved_registration', [$name]), null, null, array($result->callback_query->from->id));
                setEmployeeMenu($data[1]);

                $msg = language('admin_approved_registration_to_user');
                $msg .= "__" . language('admin_info_work_location') . $user_data['branch_name'];
                $msg .= "_" . language('admin_info_map')." /worklocation";
    ...
    /** Set employee as Admin */
        ...
        if (is_array($m) && count($m) > 0) {
            $msg = language('admin_new_already_exists', [$params['user_id']]);
        } else {
            $s = $ezzeTeamsModel->addAdmin($params);
            $msg = language('admin_new_success', [$params['user_id'], $params['name']]);
            $ezzeTeamsModel->setAdminStep($data[1], '', '');
            prepareMessage(array(array(language('button_show_user_table'), language('button_show_employee'))), "You have been added to Admin", null, null, [$data[1]]);
        }
        setAdminMenu($data[1]);
    ...
    /** Remove employee as Admin */
        ...
        if (in_array($data[1], $admin_id)) {
            ...
            $msg = language('admin_new_remove_success', [$params['user_id'], $params['name']]);
    ...
    /** Set Inactive employee */
        ...
        $msg = language('admin_inactive_user_success', [$data[1], $data[2]]);
        prepareMessage(null, $msg, null, null, array($result->callback_query->from->id));
    }
    ...
    /** Change Work Location from view Employee */
        ...
        $msg = language('admin_change_work_location', [$data[2]]);
    ...
    /* refresh employee status */
        ...
        //refresh Employee
        prepareMessage($r['keyboard'], language('your_account_refreshed'), null, null, [$data[1]]);

        //admin notify already refreshed
        $msg = language('admin_refresh_success', [$uData['firstname']." ".$uData['lastname']]);
        prepareMessage(null, $msg, null, null, array($result->callback_query->from->id));
    }
    ...
    elseif ($data[0] == 'ch-assign-branch') {
        ...
        $msg = language('admin_change_work_location_success', ['/viewEmployee'.$data[1]]);
        prepareMessage(null, $msg, null, null, array($result->callback_query->from->id));

        $msg = "Your Work Location has been changed by Admin";
        $msg .= "__" . language('admin_info_work_location').' '.$data[4];
        $msg .= "_" . language('admin_info_map') . " " . language('button_show_work_location');
        prepareMessage(null, $msg, null, null, array($data[1]));
    }
     ...
     */
        ...
        prepareMessage(null, language('admin_approved_registration'), null, null, array($result->callback_query->from->id));
        prepareMessage(generateClockInBtn(), language('admin_approved_registration_to_user'), null, null,  array($data[1]));
    }
        ...
        prepareMessage(null, language('admin_reject_registration', [$data[2]]), null, null,  array($result->callback_query->from->id));
        prepareMessage([[language('button_back_home')]], language('admin_reject_registration_to_user'), null, null,  array($data[1]));
    ...
    if ($adminUser['step'] == 'admin_branch_edit_name') {
        ...
        prepareMessage(null, language('admin_branch_add_location', [ucwords($action)]));
        return;
    }
    else if ($adminUser['step'] == 'admin_branch_remove_confirmation') {
        if (strtolower($action) == strtolower(language('button_yes'))) {
            $params = json_decode($adminUser['temp'], true);

            $ezzeTeamsModel->removeBranchById($params['id']);
            $ezzeTeamsModel->setAdminStep($userId, '');
            prepareMessage(array(array(language('button_show_user_table'))), language('admin_branch_remove_success'));
        } else if (strtolower($action) == strtolower(language('button_no'))) {
            $ezzeTeamsModel->setAdminStep($userId, '');
            prepareMessage(array(array(language('button_show_user_table'))), language('thank_you'));
        }
    }
        ...
        if (strtolower($action) == strtolower(language('button_yes'))) {
            ...
            $ezzeTeamsModel->setAdminStep($userId, '');
            $msg = language('admin_branch_edit_success');
            prepareMessage(array(array(language('button_show_user_table'))), $msg);
        } else if (strtolower($action) == strtolower(language('button_no'))) {
            $ezzeTeamsModel->setAdminStep($userId, '');
            prepareMessage(array(array(language('button_show_user_table'))), language('thank_you'));
        }
        ...
        if (count($x) == 2) {
            if ( $x[0] >= 0 && $x[0] < 24 && $x[1] >= 0 && $x[1] < 60 ) {
                ...
                $msg = language('admin_schedule_daily_report_set_data_range', [$tempData['time']]);
            } else {
                $msg = language('admin_time_invalid');
            }
        } else {
        ...
        if (is_numeric($action)){
            ...
            if ($action > 0 && $action <= $maxHour) {
                ...
                $ezzeTeamsModel->setAdminStep($userId, '', '');
                $msg = language('admin_schedule_daily_report_success', [$tempData['time'], $action]);
            } else {
