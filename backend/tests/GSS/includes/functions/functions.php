    ...
    foreach ($clockData as $clock) {
        ...
        if ($isAbsent != 'absent') {
            ...
            if ($clock[10] == 'OK' && $clock[15] == 'OK' && $clock[21] == 'OK' && $clock[26] == 'OK') {
                ...
                $arr['clockOUTEarly'] = ($clock[21] == 'EARLY CLOCK OUT' && $clock[21] != '') ? $pointCheckValue : '';
                $arr['clockOUTFailed'] = ($clock[21] == 'FAILED CLOCK OUT' && $clock[21] != '') ? $pointCheckValue : '';
                $arr['clockINWrongLocation'] = ($clock[15] == 'WRONG LOCATION' && $clock[15] != '') ? $pointCheckValue : '';
                $arr['clockOUTWrongLocation'] = ($clock[26] == 'WRONG LOCATION' && $clock[26] != '') ? $pointCheckValue : '';;
            }
        } else {
            $arr['allOK'] = '';
...
function generateDetilEmployeeBtn($user_data)
{
    ...
    $btnLine1 =
        [
            [
                "text" => language('button_change_working_time'),
    ...
    if (in_array($user_data['user_id'], $admin_id)) {
        $admin_btn = [
            "text" => language('button_remove_as_admin'),
            "callback_data" => "remove-admin_" . $data
        ];
        array_push($btnLine1, $admin_btn);
    } else {
        $admin_btn = [
            "text" => language('button_set_as_admin'),
            "callback_data" => "set-admin_" . $data
        ];

        $refresh_btn = [
            "text" => language('button_refresh'),
    ...
    if ($user_data['approval_status'] == 'approved') {
        $user_btn = [
            "text" => language('button_change_work_location'),
            "callback_data" => "change-work-location_" . $data
        ];
        array_push($btnLine2, $user_btn);
        $user_btn = [
            "text" => language('button_set_inactive'),
...
function generateApprovedEmployee($chat_id = null, $is_edit = false, $limit = 10, $offset = 0, $page = 1)
{
    ...
    $list = "<strong><u>" . language('admin_list_active_employee') . "</u></strong>";
    ...
    if (is_array($lastClock) && $lastClock['is_clock_in'] == 'clock_in') {
        ...
        if ($timeNow < $dateEndStr) {
            ...
            if ($timediffFromLastClockOut > $hourMaxLate && $uData['step'] == 'clock_in_done') {
                ...
                prepareMessage($r['keyboard'], language('please_refresh'));
                exit;
                $isWorking = false;
    ...
    if ($user_data['step'] == 'clock_in_done') {
        ...
        if ($botSettings['module_break'] == 1) {
            if ($user_data['can_break'] == 1) {
                array_push($keyboard, [language('button_start_break')]);
            }
        }
        if ($botSettings['module_visit'] == 1) {
            if ($user_data['can_visit'] == 1) {
                array_push($keyboard, [language('button_start_visit')]);
            }
        }
        //$keyboard = array(array(language('button_start_break')), array(language('button_clock_out')));
        array_push($keyboard, [language('button_clock_out')]);
        $msg = language('already_clock_in');
    } elseif ($user_data['step'] == 'on_break') {
        $keyboard = array(array(language('button_end_break')), array(language('button_clock_out')));
        $msg = language('already_break');
    } elseif ($user_data['step'] == 'on_visit') {
        $keyboard = array(array(language('button_end_visit')), array(language('button_clock_out')));
        $msg = language('already_break');
    } else {
        $keyboard = generateClockInBtn();
        $msg = language('option_menu');
    }
...
function generateClockInStatusMSG($clock_in_data, $header)
{
    ...
    if ($time2 > $tolleranceH) {
        ...
        $m = $diff . " " . language('admin_info_late');
    } elseif ($time2 < $tolleranceL) {
        $diff = ($tolleranceL - $time2) / 60;
        $diff = convertMinToReadable($diff);
        $m = $diff . " " . language('admin_info_early');
    } else {
        $m = language('admin_info_within_tollerance');
    }
    $tgUsername = (isset($clock_in_data['tgusername'])) ? $clock_in_data['tgusername'] : "@".str_replace("_", "###", $userProfile['tg_username']);

    $msg = $header .
        "__" . language('admin_info_job_description') . $userProfile['jobdesc'] .
        "_" . language('admin_info_work_location') . $userProfile['branch_name'] .
        "__" . language('admin_info_first_name') . $userProfile['firstname'] .
        "_" . language('admin_info_last_name') . $lastname .
        "_" . language('admin_info_tg_username') . $tgUsername .
        "_" . language('admin_info_phone_number') . $phone .
        "_" . language('admin_info_employee_id') . $eId .
        "__" . language('admin_info_sch_clock_in') . $clock_in_data['work_start_time'] .
        "_" . language('admin_info_actual_clock_in') . $clock_in_data['clock_in_time'] .
        "_" . language('admin_info_diff_time') . $m .
        "__" . language('admin_info_distance') . $distance .
        "_" . language('admin_info_map') . "/showMapClock" . $clock_in_data['id'] .
        "_" . language('admin_info_employee') . "/viewEmployee" . $clock_in_data['user_id'];
...
function generateClockOutStatusMSG($clock_in_data, $header)
{
    ...
    $msg = $header .
        ...
        "__" . language('admin_info_first_name') . $userProfile['firstname'] .
        "_" . language('admin_info_last_name') . $lastname .
        "_" . language('admin_info_tg_username') . $clock_in_data['tgusername'] .
        "_" . language('admin_info_phone_number') . $phone .
        "_" . language('admin_info_employee_id') . $eId .
        "__" . language('admin_info_sch_clock_out') . $clock_in_data['work_start_time'] .
        "_" . language('admin_info_actual_clock_out') . $clock_in_data['clock_in_time'] .
        "_" . language('admin_info_diff_time') . $m .
        "__" . language('admin_info_distance') . $distance .
        "_" . language('admin_info_map') . "/showMapClock" . $clock_in_data['id'] .
        "_" . language('admin_info_employee') . "/viewEmployee" . $clock_in_data['user_id'];
...
function generateReceivedNewApplicationBtn($user_data)
{
    ...
    return [
        [
            "text" => language('button_approve'),
            "callback_data" => "approve_" . $data
        ],
        [
            "text" => language('button_reject'),
...
function generateStartEndTimeBtn($id, $name, $day)
{
    return [
        [
            "text" => language('button_set_startend_time'),
...
function generateWorkingTimeBtn($id, $day, $start_time, $h = '', $mn = '')
{
    ...
    if ($start == '-end') {
        $submit_button =
            [
                [
                    "text" => language('button_confirm_approve'),
...
{
    return language('admin_new_registration');
}
...
function generateListEmployee($chat_id = null, $is_edit = false, $limit = 10, $offset = 0, $page = 1)
{
    ...
    $list = "<strong><u>" . language('admin_list_employee_registration') . "</u></strong>";
